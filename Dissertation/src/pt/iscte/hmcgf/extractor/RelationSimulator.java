...
public class RelationSimulator
{
	...
	public void simulate(String namespace, RelationStorage storage, int numLines)
	{
		...
		// step 2. with the produced type get relations also sorted by outgoing types (unique)
	...
	private void processRelations(List<Relation> relations)
	{
		...
		else
		{
			...
			while (choice < 1 || choice > 5 || choice >= relations.size());
		}
	...
	public class ReflectionSimulatorComparator implements Comparator<Relation>
	{
		...
		{
			return RelationStorage.getUniqueOutgoingTypesForRelationshipSet(storage.getOutgoingRelationsForType(r2.getDestination())).size()
					- RelationStorage.getUniqueOutgoingTypesForRelationshipSet(storage.getOutgoingRelationsForType(r1.getDestination())).size();
		}
	}
